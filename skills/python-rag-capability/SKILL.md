...
### 8.8 repository 实体标准化约束
1. `app/rag/repository/` 中每张核心控制面表都应有明确的持久化实体对象，不应长期把整条数据库记录表示为裸 `dict`。
2. 推荐使用“ORM 持久化实体对象 + mapper + 领域对象 / read model”的组织方式。
3. `repository` 对外返回值应优先收敛为领域对象或专用 read model，而不是把 ORM 记录或裸 `dict` 广泛泄漏给上层。
4. `*_details` 这类半结构化字段内部允许继续使用 `dict`，但查询结果整体不得长期维持为裸 `dict`。
...
rag 提供给 services 的正式查询结果应优先为领域对象或 read model，而不是持续输出裸 `dict`。
