...
### 4.4 当前代码没有 citations 输出
当前 `/chat` 与 `/chat_stream` 都还没有 citations 字段。
未落地能力不得写成默认基线。
...
本 skill 的目标，是确保 `app/api/` 在当前项目中持续作为 HTTP / SSE 接入层演进，稳定承接现有 chat / stream / cancel / reset / health 协议，而不是把尚未落地的 citations 或 retrieval 语义提前写进 API 完成态。
