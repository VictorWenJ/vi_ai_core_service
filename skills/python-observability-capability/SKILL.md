...
当前代码中，观测重点仍是 request / stream / context assembly / provider 调用。
retrieval / citation 观测当前尚未落地。
...
- 当前不包含 retrieval / citation 专项可观测性
...
### 8.5 Phase 6 约束
当前代码尚未落地 retrieval / citation 观测；
不得把这些字段写成现有必备能力。
...
3. 是否保证 JSON-safe？
4. 是否避免了敏感正文泄漏？
5. 是否没有把未落地 retrieval / citation 字段写成已实现事实？
6. 是否补了测试？
...
本 skill 的目标，是确保 `app/observability/` 在当前项目中持续作为结构化日志基础设施层演进，以 `log_report` 和 JSON-safe 序列化支撑已落地主链路，而不是把尚未出现的 retrieval / citation 观测写成默认能力。
