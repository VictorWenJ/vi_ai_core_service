...
## 4. 当前阶段核心页面范围
当前页面范围不扩张，但需适配后端从内存控制面切换为 MySQL 控制面的查询语义。
...
### 5.3 前后端参数一致性
控制台页面、hook、API client、类型定义与后端 request contract 必须保持参数一致。

具体要求：
- 后端接口存在的 request 参数，前端不得无故遗漏
- 前端表单输入项、hook 状态、API client 传参、类型定义应保持一一对应
- 不允许出现“后端已支持字段，但页面没有输入项、hook 没有状态、API 没有透传”的断链
- 当后端新增或调整 request 参数时，必须同步检查并更新：
  - 页面表单
  - hook / 状态管理
  - API client
  - 类型定义
  - 必要的默认值与校验逻辑
- 不要通过前端私自硬编码默认值来掩盖契约不一致问题，除非后端 contract 已明确允许
...
前端 request 参数链路必须完整覆盖后端已公开 contract，不得只接一部分字段就默认“页面已完成”。
