...
- 外部 LLM 二次摘要链路与异步 memory worker
...
### 阶段四-D：Context Engineering Phase 4（分层短期记忆）

#### 目标
在不引入长期记忆 / RAG / 向量检索的前提下，把当前上下文能力从“持久化历史窗口”升级为“conversation-scoped layered short-term memory”。

#### 本阶段必须完成
- 上下文主作用域统一为 `(session_id, conversation_id)`
- `ContextWindow.messages` 的语义收敛为 **recent raw messages**，不再表示全部历史
- 新增 `rolling_summary` 持久化层
- 新增 `working_memory`（结构化短期记忆）持久化层
- request assembly 能按固定顺序读取并装配：
  - system prompt
  - working memory block
  - rolling summary block
  - recent raw messages
  - current user input
- response-time 能完成 recent raw 更新、summary compaction、working memory reducer 更新与持久化
...
4. 在没有文档约束前大规模重构
5. 在模块边界不清晰时继续快速叠功能
6. 把 Phase 4 直接做成长期记忆或 RAG 工程
7. 把 Docker / infra 做成新的业务层
8. 为 working memory 一上来引入复杂语义抽取平台
9. 在没有 scope 隔离前就堆更多记忆层
...
4. 再落地 context / services / API / tests 代码
5. 保持 `infra/` 运行方式不被业务改造污染
6. 最后补测试与文档回写
...
- 上下文主作用域升级为 `(session_id, conversation_id)`
- recent raw / rolling summary / structured working memory 三层短期记忆落地
- request-time 读取、response-time 更新、持久化与 trace 闭环成立
- `app/rag/` / 长期记忆边界被清晰预留且未被越界实现
...
Phase 2 已让请求时上下文治理成型；Phase 3 已将短期会话能力升级为持久化 session memory；当前 Phase 4 已将其升级为 **分层短期记忆**。
...
- 未明确 `(session_id, conversation_id)` scope 语义，不进入 layered memory 开发
- 未明确 `ContextWindow.messages` 语义收敛为 recent raw，不进入模型改造
- 未明确 `ContextMemoryConfig` 与现有 config 边界，不进入配置改造
- 未明确 request-time / response-time 链路，不进入 service 与 assembler 改造
- 未更新文档与 skill，不进入 Phase 4 实现
