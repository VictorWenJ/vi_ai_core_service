...
- 本层是横切基础设施层，不承载业务流程
- 当前阶段仅保留 `log_until.py` 的最小实现，不做 tracing/metrics/alerting 平台化建设
- 当前阶段业务 payload 默认可输出（调试优先）
- `LOG_*` 配置项在 `AppConfig` 中保留，但尚未接入日志运行时开关
...
### 5.2 Context Engineering Phase 2 调用链补充

在 Context Engineering Phase 2 中，主链路在 Phase 1 基础上引入 token-aware 窗口选择与截断策略、摘要/压缩策略，以及重置会话接口。调用链整体保持不变，但上下文治理细节扩展为：

```
api -> services/chat_service.py -> services/request_assembler.py
   -> context/manager.py -> context/stores/* -> context/policies/*
   -> context/policies/token_aware_window_selection.py
   -> context/policies/token_aware_truncation.py
   -> context/policies/summary.py
   -> services/request_assembler.py -> providers/*
```

#### 设计说明

1. context 层新增 token-aware 策略接口，能够在历史消息量大于预算时根据 token 数量选择窗口并裁剪。
2. 新增 `SummaryPolicy` / `CompactionPolicy`，当 truncation 无法满足预算时，通过摘要或压缩策略生成浓缩历史，以保持上下文完整性。
3. `request_assembler.py` 负责驱动新的 token-aware pipeline，汇总 system prompt、摘要信息、selected history 与当前用户输入装配为最终请求。
4. API 层新增 reset/clear 路由，调用 context manager 清空会话历史；reset 动作不属于 context 层自动行为，由应用编排层显式触发。
5. 架构边界继续保持：context 层不负责最终 prompt 装配顺序或 provider 请求格式。

#### 边界说明

- token-aware 逻辑不应写进 `chat_service.py`，应由 context policy 实现。
- 摘要策略不负责决定系统 prompt 或 messages 顺序；总结文本仅作为历史的一部分由 `request_assembler.py` 插入。
- reset 行为属于服务层与 API 层协作，不应在 context 存储实现中隐藏。
...
- 本阶段已实现并验收：
  ...
  - 可回归的最小测试集
- 本阶段正在增强并要求边界正确：
  - short-term conversation history governance（短期会话历史治理）
  - token-aware window selection（默认）
  - token-aware truncation（默认）
  - deterministic summary/compaction（默认）
  - history serialization pipeline（历史序列化管线）
  - fallback 行为分流（`summary_then_drop_oldest` 与 `drop_oldest`）
  - request assembly 中的上下文装配治理
- 本阶段仅预留，不要求落地：
  - streaming 真正实现
  - 多模态真实处理链路
  - tools/function calling 与 结构化输出 真正能力
  - persistence / retrieval-backed summary memory / semantic recall（持久化/检索增强摘要记忆/语义召回）
  - RAG retrieval pipeline（RAG 检索管线）
  - 新系统层与重平台化改造
...
在 Context Engineering Phase 1 中，重点是把“服务端历史如何进入一次模型请求”收敛为正式架构路径；在 Context Engineering Phase 2 中，进一步通过 token-aware 策略和摘要/压缩策略精准控制历史窗口与预算，并提供会话重置能力，但仍然不引入长期记忆或 RAG 平台。
...
### Context Engineering Phase 2 专项执行顺序（新增）

1. 先明确 token 预算来源和默认值
2. 再定义 `TokenAwareWindowSelectionPolicy`、`TokenAwareTruncationPolicy`、`SummaryPolicy` 接口
3. 再更新 `ContextPolicyPipeline` 以组合这些策略
4. 再升级 `request_assembler.py` 以驱动 token-aware pipeline，并暴露 assembly trace
5. 增加 reset/clear API 与 context manager 清空方法
6. 最后补充策略行为的测试与文档回写
