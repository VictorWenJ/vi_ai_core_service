...
Phase 7 中，本模块继续维持在线主链路稳定，不承担 benchmark 或离线构建主实现。
...
9. 对系统运行态摘要等跨模块信息提供面向消费者无关的应用层聚合（若当前阶段需要）
...
- runtime summary / config summary / health 等系统摘要能力若需在应用层聚合，应位于 `app/services/` 这类应用编排层，而不是长期放在 `app/rag/console_service.py` 中
- `services` 可以承接消费者无关的应用级聚合，但不得演化为跨全系统的 God Service

### 6.8 repository 返回值消费原则
- `services` 层不应长期以字符串键方式消费 repository 返回的裸 `dict` 作为主要业务输入。
- `services` 应优先消费领域对象或专用 read model；若当前阶段存在历史 `dict` 结果，应以增量方式逐步迁移。
- `*_details` 这类半结构化字段内部允许继续使用 `dict`，但服务主编排逻辑不应建立在“整条结果都是裸 `dict`”的前提上。
...
10. 不允许按消费者命名新的应用服务文件或类，例如长期维持 `*_console_service`。
11. 不允许在 service 主编排逻辑中大量使用字符串键访问 repository 返回值，例如 `result["status"]`、`row["build_id"]` 这类模式长期存在。
...
6. `ChatService` / `StreamingChatService` 的职责是否仍清晰？
7. 是否没有把向量库或 embedding 厂商细节直接暴露到 service 业务逻辑中？
8. services 是否优先消费领域对象 / read model，而不是持续依赖 repository 裸 `dict` 返回值？
8. retrieval / citation 是否严格来自 rag runtime，而不是 services 内部拼接？
9. runtime summary / config summary / health 等跨模块摘要能力是否落在合理的应用层聚合位置？
