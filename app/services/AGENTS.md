...
### 6.5 当前代码尚未落地 retrieval 编排
- 当前 `services` 还没有 `app/rag/` 运行时接入
- 当前 request assembly 也没有 knowledge block
- 若后续进入 Phase 6，实现必须在本层统一编排，不能散落到 API / provider / context
...
- `chat_from_user_prompt` / `stream_chat_from_user_prompt` 与 `LLMRequest` 主入口并存

当前代码事实补充：

- `ChatRequestAssembler` 当前装配顺序为：system -> working memory -> rolling summary -> recent raw -> user
...
- 当前代码尚未实现 retrieval 调用、knowledge block 注入或 citations 输出
...
5. 不允许把尚未实现的 citations / retrieval 写成已存在行为
...
`app/services/` 在当前代码基线中是系统的应用编排中枢，负责在不破坏 Phase 4 / Phase 5 已有同步与流式会话主链路的前提下，把 context、prompts、providers 与取消 / 生命周期收口统一编排进 chat core，而当前尚未接入 retrieval、knowledge block 或 citations，并在后续更新中严格遵守模块文档的模板冻结规则。
