...
- `chat/base.py`：chat Provider 抽象与共享异常
- `chat/openai_compatible_base.py`：OpenAI 兼容调用基类
- `chat/openai_provider.py` / `chat/deepseek_provider.py`：已实现 chat Provider
- `chat/gemini_provider.py` / `chat/doubao_provider.py` / `chat/tongyi_provider.py`：chat 脚手架 Provider
- `chat/registry.py`：chat Provider 注册与成熟度描述
- `embeddings/base.py`：embedding provider 抽象
- `embeddings/deterministic_provider.py` / `embeddings/openai_provider.py`：embedding provider 实现
- `embeddings/registry.py`：embedding provider 工厂
...
7. 维护独立 embedding provider 抽象与实现入口
...
- embedding 通过独立 `BaseEmbeddingProvider` 抽象接入，不污染 `BaseLLMProvider`
- 当前目录结构按能力分层为 `chat/` 与 `embeddings/`，embedding provider 实现仍归属 `app/providers/` 而非 `app/rag/`
...
- 当前仓库已落地 embedding provider 抽象、实现与测试
- embedding provider 当前支持 `deterministic` 与 `openai`
...
7. 不允许强制所有 chat provider 同时实现 embedding 契约
8. provider 相关 dataclass 字段必须逐项补充中文注释，说明字段语义、默认行为与是否可空。
9. 与 provider、embedding、registry 相关的默认配置项必须补充中文注释；涉及 timeout、dimension、top-k、threshold 等数值时必须明确单位或语义。
10. 不允许删除仍在使用的 provider/config 中文注释。
...
6. embedding 抽象是否保持独立且不污染 `BaseLLMProvider`？
7. 错误映射是否合理？
8. 是否没有把底层厂商响应结构直接泄漏给上层？
...
8. embedding provider 构建与调用测试

---

## 12. 一句话总结

`app/providers/` 在当前代码基线中是系统的模型与厂商接入层，负责以统一抽象对接 chat completion、stream completion 与独立 embedding provider，并通过注册表管理已实现与脚手架 Provider，而不参与业务编排、知识检索或 citation 逻辑，并在后续更新中严格遵守模块文档的模板冻结规则。
