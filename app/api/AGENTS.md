...
7. 正确承接 `session_id` / `conversation_id` 等会话标识
...
7. 不负责决定 working memory / summary / history 的装配顺序
...
Phase 4 不新增 conversation CRUD 全家桶，也不新增 memory admin/debug 接口。
...
### 7.2 layered memory 细节不进入路由层

Phase 4 中 API 层可以继续承接与 session / conversation 相关的输入，但绝不直接操作 persistent store、summary、working memory 或 reducer 细节。

### 7.3 响应结构稳定

同一类接口响应结构应保持稳定，不随内部实现频繁变更。

### 7.4 `conversation_id` 的语义必须清晰

- API 层负责把用户输入中的 `conversation_id` 明确透传给 service
- API 层不负责生成 store 内部默认 scope
- API 层不应把 `(session_id, conversation_id)` 的持久化细节暴露为外部协议复杂度
...
- 已实现并验收：
  ...
  - 基础输入校验与错误映射
- 当前本轮要保持：
  - `/chat` 与 `/chat/reset` 在 Phase 4 layered memory 场景下保持契约稳定
  - API 层继续通过 service 调用，不直接触达 Redis/store 细节
  - 继续正确承接 `session_id` / `conversation_id`
- 当前不要求落地：
  - streaming 接口
  - 多模态接口
...
2. 不允许在路由里拼接 Redis key、设置 TTL 或写默认 scope 规则
3. 不允许在路由里手写上下文治理逻辑
...
3. 请求/响应结构是否稳定？
4. 错误信息是否清晰？
5. `conversation_id` 与 `session_id` 是否被正确透传？
...
6. layered memory 落地后 `/chat` 仍保持既有响应契约
...
进入 Phase 4 后，这一点不变；变化的是服务层和 context 层会在 API 背后升级为 conversation-scoped layered short-term memory，但这些细节不应泄漏到路由层。
