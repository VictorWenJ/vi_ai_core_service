...
- citation schema 当前不在本模块，已在 `app/api/schemas/chat.py` 落地
...
7. 所有 `@dataclass` 契约字段都必须逐项补充中文注释，明确字段语义；不得只依赖字段名表达含义。
8. 当字段的可空性、默认值、取值语义或兼容性约束发生变化时，必须同步更新对应中文注释。
9. 不允许删除仍然存在的 schema 字段中文注释。
